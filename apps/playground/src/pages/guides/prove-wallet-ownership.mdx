...
const signature = await wallet.signData("meshjs's core-cst is pure typescript while core-csl is wasm rust.");
...
return (
  ...
  >
    <>
      ...
      <CardanoWallet />
      <Button onClick={() => runDemo()} style={loading ? "warning" : "light"} disabled={loading || !connected}>Sign Message</Button>
    </>
  </LayoutImageHeaderAndBody>
  );
}
