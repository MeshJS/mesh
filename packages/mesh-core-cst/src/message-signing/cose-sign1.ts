...
class CoseSign1 {
  ...
  constructor(payload: {
    ...
    if (
      !this.unProtectedMap.map.find(
        (value) =>
          JSON.stringify(value.k) === JSON.stringify(new CborText("hashed")),
      )
    ) {
  ...
  hashPayload() {
    ...
    if (
      hashed &&
      JSON.stringify(hashed.v) === JSON.stringify(new CborSimple(true))
    )
  ...
  getAddress(): Buffer {
    const address = this.protectedMap.map.find((value) => {
      JSON.stringify(value.k) === JSON.stringify(new CborText("address"));
    });
  ...
  getPublicKey(): Buffer {
    const publicKey = this.protectedMap.map.find((value) => {
      JSON.stringify(value.k) === JSON.stringify(new CborUInt(4));
    });
...
const getPublicKeyFromCoseKey = (cbor: string): Buffer => {
  ...
  const publicKeyEntry = decodedCoseKey.map.find((value) => {
    JSON.stringify(value.k) === JSON.stringify(new CborNegInt(-2));
  });
