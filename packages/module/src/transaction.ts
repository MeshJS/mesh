...
export class Transaction {
  ...
  private async _getTxBuilderConfig() {
    const protocolParameters = await this._blockfrost.epochsLatestEpochProtocolParameters();
  ...
  /*private async _addPaymentAda({
    ...
    txBuilder.add_output(
      SerializationLib.TransactionOutput.new(
        address,
        SerializationLib.Value.new(StringToBigNum(lovelace.toString()))
      )
    );
  }*/
  ...
  /*private async _addOutputsJustADA({ txBuilder, outputs }: { txBuilder: any, outputs: any }) {
    await Promise.all(
  ...
  payToAddress(txBuilder: any, address: any, assets: any) {
    const output = SerializationLib.TransactionOutput.new(
      SerializationLib.Address.from_bech32(address),
