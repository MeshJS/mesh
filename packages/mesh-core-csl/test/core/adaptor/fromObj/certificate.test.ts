...
describe("Certificate Adaptor - fromObj", () => {
  test("Basic Certificate - RegisterStake", () => {
    ...
    const cert = certificateFromObj(objRepresentation);
  ...
  test("Round-trip conversion test", () => {
    ...
    const originalCert: Certificate = {
      type: "BasicCertificate",
      certType: {
        type: "StakeAndVoteDelegation",
        stakeKeyAddress:
    ...
    const objRepresentation = certificateToObj(originalCert);
    const roundTripCert = certificateFromObj(objRepresentation);

    // Verify the certificate is the same after round-trip conversion
    expect(roundTripCert).toEqual(originalCert);
  });
});
