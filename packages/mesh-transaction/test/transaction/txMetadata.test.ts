...
describe("Transaction Metadata Merge", () => {
  ...
  it("should not merge two different numbers", () => {
    ...
    expect(() => mergeContents(currentMetadata, newMetadata, 1)).toThrow(
      "cannot merge 42 with 43",
    );
  });
  ...
  it("should return the latest item if there is no merge", () => {
    ...
    expect(mergeContents(currentMetadata, newMetadata, 0)).toEqual(
      expectedOutput,
    );
  });
  ...
  it("should not merge two different values of the same object key", () => {
    ...
    expect(() => mergeContents(currentMetadata, newMetadata, 2)).toThrow(
      "cannot merge 1 with 2",
    );
  });
  ...
  it("should not merge different types", () => {
    expect(() =>
      mergeContents(metadataObjToMap(0), metadataObjToMap([]), 1),
    ).toThrow("cannot merge primitive type with array type");
    expect(() =>
      mergeContents(metadataObjToMap({}), metadataObjToMap(""), 1),
  ...
  it("plain object to map conversion should not allow nullish values", () => {
    ...
    expect(() => metadataObjToMap({ value: null })).toThrow(
  ...
  it("should merge multiple CIP-25 NFTs metadata under different policy ids", () => {
    const currentMetadata = metadataObjToMap({
      policyId1: {
        "My NFT 1": {
          name: "My NFT 1",
          files: [{ name: "NFT 1 Image", src: "xyz", mediaType: "image/jpeg" }],
        },
      },
    });
    const newMetadata1 = metadataObjToMap({
      policyId2: {
        "My NFT 1": {
          ...
          files: [{ name: "NFT 1 P 2", src: "abc", mediaType: "image/png" }],
        },
      },
    });
    ...
    const newMetadata2 = metadataObjToMap({
      policyId1: {
        "My NFT 2": {
          name: "My NFT 2",
          files: [{ name: "NFT 2 Image", src: "pqr", mediaType: "image/jpeg" }],
        },
      },
    });
  ...
  it("should preserve metadata entries with other tags in the original order", () => {
    ...
    const expectedOutput = new Map([
      ...
      [
        721n,
        metadataObjToMap({
          policyId1: { NFT1: { name: "line 3" }, NFT2: { name: "line 5" } },
        }),
      ],
      [1n, metadataObjToMap("line 4")],
    ]);
