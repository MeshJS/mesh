...
import { LargestFirstInputSelector, MeshTxBuilder } from "@meshsdk/transaction";
...
describe("MeshTxBuilder - coin selection - Largest first", () => {
  ...
  it("Insufficient funds for token", async () => {
    ...
    await expect(
      txBuilder
        ...
        .changeAddress(baseAddress(1))
        .complete(),
    ).rejects.toThrow(`Not enough UTxOs to cover the required value.`);
  });
