...
export class Wallet {
   ...
   * load encryptedRootKey wallets
   * sign tx with encryptedRootKey wallets
   */
  ...
  encryptWithPassword = async (password, rootKeyBytes) => {
    ...
    return SerializationLib.Instance.encrypt_with_password(
      passwordHex,
      salt,
      nonce,
      rootKeyHex
  ...
  decryptWithPassword = async (password, encryptedKeyHex) => {
    ...
    let decryptedHex;
    try {
      decryptedHex = SerializationLib.Instance.decrypt_with_password(
        passwordHex,
        encryptedKeyHex
      );
    } catch (err) {
  ...
  requestAccountKey = async (encryptedRootKey, password, accountIndex = 1) => {
    let accountKey = SerializationLib.Instance.Bip32PrivateKey.from_bytes(
      Buffer.from(
  ...
  async createWalletMnemonic({ seedPhrase, password }) {
    ...
    let rootKey = SerializationLib.Instance.Bip32PrivateKey.from_bip39_entropy(
  ...
  async loadWallet({
    ...
    const paymentAddr = SerializationLib.Instance.BaseAddress.new(
      network,
      SerializationLib.Instance.StakeCredential.from_keyhash(paymentKeyHash),
      SerializationLib.Instance.StakeCredential.from_keyhash(stakeKeyHash)
    )
      .to_address()
      .to_bech32();
  ...
  async signTxWithPassword({
    ...
    const rawTx = SerializationLib.Instance.Transaction.from_bytes(
      Buffer.from(tx, "hex")
    );

    const txWitnessSet = SerializationLib.Instance.TransactionWitnessSet.new();
    const vkeyWitnesses = SerializationLib.Instance.Vkeywitnesses.new();
    const txHash = SerializationLib.Instance.hash_transaction(rawTx.body());
    ...
    keyHashes.forEach((keyHash) => {
      ...
      const vkey = SerializationLib.Instance.make_vkey_witness(
        txHash,
        signingKey
      );
